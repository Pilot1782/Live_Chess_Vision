...
def resizeAsNeeded(img, max_size=(2000, 2000), max_fail_size=(2000, 2000)):
    """Resize image if larger than max_size, but not if larger than max_fail_size"""
    ...
    if img.size[0] > max_size[0] or img.size[1] > max_size[1]:
        ...
        if img.size[0] > img.size[1]:
            ...
            ratio = np.float32(new_size) / img.size[0]
        else:
            # resize by height
            ratio = np.float32(new_size) / img.size[1]
        # print("Reducing by factor of %.2g" % (1. / ratio))
        new_size = (np.array(img.size) * ratio).astype(int)
        # print("New size: (%d x %d)" % (int(new_size[0]), int(new_size[1])))
